...
/*   Updated: 2023/06/24 22:38:11 by ylaaross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_strlen_m(char *p, int i, int *v)
{
	...
	if(p[i] == '"' || p[i] == '|' || p[i] == '\'' || p[i] == ' '|| p[i] == '<' || p[i] == '>' || p[i] == 9 || p[i] == 11 || (p[i] == '$'  && (p[i + 1] && (!(p[i + 1] >= '0' && p[i + 1] <= '9') && p[i + 1] != '@'))))
	{
	...
	while (p[i] && !(p[i] == '"' ||p[i] == '|' || p[i] == '<' || p[i] == '>' || p[i] == '\'' || p[i] == ' ' || p[i] == '$' || p[i] == 9 || p[i] == 11 ))
	{
		*v = WORD;
		counter++;
		i++;
